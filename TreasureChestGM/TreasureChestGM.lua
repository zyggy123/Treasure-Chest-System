--------------------------------------------------------------------------
-- Treasure Chest GM Control
-- Companion client addon for zyggy123/Treasure-Chest-System (AzerothCore/Eluna)
-- https://github.com/zyggy123/Treasure-Chest-System
--
-- The server module hooks PLAYER_EVENT_ON_CHAT (event 18), which only fires
-- for normal public chat (SAY/YELL). This addon just builds the "#chest ..."
-- command strings and sends them via SendChatMessage(), so the GM doesn't
-- have to type them by hand. All server-side validation (GM level check,
-- item ID lookup, etc.) still happens exactly as before -- this is purely
-- a convenience/UI layer, no server changes needed.
--
-- IMPORTANT: because the hook only listens on SAY/YELL, commands sent from
-- this panel are visible to any nearby player, exactly as if you had typed
-- them yourself. There is no whisper/party/guild variant available unless
-- the server script is edited to also register PLAYER_EVENT_ON_WHISPER.
--------------------------------------------------------------------------

local CHAT_CHANNEL = "SAY" -- keep as SAY or YELL -- server only listens on PLAYER_EVENT_ON_CHAT
...
local function SendChestCommand(cmd)
    SendChatMessage(cmd, CHAT_CHANNEL)
    Print("Sent: " .. cmd)
end
...
local function UpdateMinimapPos()
    local angle = math.rad(TreasureChestGM_DB.minimapAngle)
...
eventFrame:SetScript("OnEvent", function(self, event, addonName)
    if addonName == "TreasureChestGM" then
...
Print("loaded. Type /tcs to open the panel, or /tcs minimap to toggle the icon.")
